...
The app is `LSUIElement` (agent app: no Dock icon, no app menu bar) and runs unsandboxed (`ENABLE_APP_SANDBOX` removed from the `swiss_bar` target) — several features (window switcher, keyboard cleaning mode) need a global `CGEventTap` and cross-process Accessibility (`AXUIElement`) control that don't work under App Sandbox. The `swiss_barWidgets` extension target is the one exception: WidgetKit extensions are always sandboxed (no opt-out), so it and the main app share Claude usage data via a plain JSON file under `~/Library/Application Support/<bundle-id>/ClaudeUsage/` rather than in-process — see `swiss_bar/ClaudeUsage/ClaudeUsageSharedStore.swift`. This is **not** an App Group container: that was the first approach, and it looked correct (entitlements present, properly provisioned, unsandboxed app could write it) but the sandboxed widget's reads of a file the unsandboxed app wrote were denied at the sandbox layer on-device (macOS 26.5.2) — a real, reproducible limitation of mixing an unsandboxed writer with a sandboxed reader on one App Group container, not a config mistake. `swiss_barWidgets.entitlements` instead grants the widget read-only access to that one directory via `com.apple.security.temporary-exception.files.home-relative-path.read-only`, which is keyed on path rather than App Group provenance and isn't subject to the same denial. `swiss_bar/swiss_bar.entitlements` has no entitlements at all (the app is unsandboxed and needs none) — never add `app-sandbox` there, it would break the window switcher and keyboard cleaning. Distribution is via GitHub Releases, signed with a self-signed certificate (not an Apple Developer ID) — not the Mac App Store, not notarized. See `INSTALL.md` for why, and the Releasing section below for how.
